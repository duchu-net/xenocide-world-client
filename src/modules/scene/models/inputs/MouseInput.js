...
class MouseInput extends Module {
  ...
  dispatchAction(action) {
    if (this._action) this._action(action)
  }

  changeCameraTo(newCamera) {
		this._camera = newCamera
	}

  ready(scene, container, camera, action = null) {
  ...
  _intersectAndDispatch(callbackName, mouseEvent) {
    ...
    ReactUpdates.batchedUpdates(() => {
      for (let i = 0; i < intersections.length; ++i) {
        ...
        // DUCHU ADDED
        if (callbackName == 'onMouseDown' && object.userData._model)
          this.dispatchAction(object.userData._model)
        // END DUCHU ADDED
